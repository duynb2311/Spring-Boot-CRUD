...
public class CauThuServiceImpl implements CauThuService {
    ...
    public ResponseWithCollectionDto<List<CauThu>> getCauThuByCauLacBo(String club, Integer page, Integer size){
        if(Objects.isNull(club)||Objects.isNull(page)||Objects.isNull(size)){
            throw new IllegalArgumentException();
        }
        ...
        if(!cauThus.isEmpty()){
            ...
            return new ResponseWithCollectionDto<>(HttpStatus.NOT_FOUND.value(), MainConstants.API_GET_CAU_THU_BY_CLB_NOT_FOUND_MESSAGE, cauThus);
        }
    }
    ...
    public ResponseWithObjectDto<CauThu> updateCauThu(CauThu cauThu){
        ...
        if(Objects.nonNull(currentCauThu)){
